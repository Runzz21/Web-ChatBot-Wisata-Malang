...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
    <div>
        ...
        <h1 class="text-2xl font-display font-semibold text-ink mt-1.5 tracking-tight">Data Kategori</h1>
        <p class="text-ink/40 mt-1">Kelola kategori destinasi wisata</p>
    </div>
    <a href="{{ route('admin.kategori.create') }}" class="btn-primary text-sm group">
            ...
            <table class="w-full text-sm text-left">
                <thead>
                    <tr class="border-b border-ink/10">
                        ...
                        <th class="py-3.5 px-4 font-semibold text-leaf-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Nama Kategori</th>
                        <th class="py-3.5 px-4 font-semibold text-leaf-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Slug</th>
                        <th class="py-3.5 px-4 font-semibold text-leaf-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Warna</th>
                        <th class="py-3.5 px-4 font-semibold text-leaf-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Icon</th>
                        <th class="py-3.5 px-4 font-semibold text-leaf-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Jumlah</th>
                        <th class="py-3.5 pl-4 font-semibold text-leaf-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($kategori as $item)
                        <tr class="border-b border-ink/5 hover:bg-leaf-600/5 transition-colors duration-200">
                            ...
                            <td class="py-3.5 px-4 font-medium text-leaf-700 whitespace-nowrap">{{ $item->nama_kategori }}</td>
                            <td class="py-3.5 px-4 text-ink/40 font-mono text-[0.75rem] whitespace-nowrap">{{ $item->slug }}</td>
                            <td class="py-3.5 px-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="w-5 h-5 rounded-lg" style="background-color: {{ $item->warna_badge }}; border: 1px solid rgba(14, 29, 24, 0.08);"></span>
                                    <span class="text-ink/40 font-mono text-[0.6875rem]">{{ $item->warna_badge }}</span>
                                </div>
                            </td>
                            <td class="py-3.5 px-4 text-ink/50 whitespace-nowrap">
                                @if($item->icon)
                                    <code class="text-[0.75rem] font-mono text-ink/40">{{ $item->icon }}</code>
                                @else
                                    <span class="text-ink/25">-</span>
                                @endif
                            </td>
                            <td class="py-3.5 px-4 font-mono text-[0.75rem] text-ink/50 whitespace-nowrap">{{ $item->destinasi_count ?? $item->destinasions->count() }}</td>
                            <td class="py-3.5 pl-4 whitespace-nowrap">
                                <div class="flex items-center gap-0.5">
                                    <a href="{{ route('admin.kategori.edit', $item) }}"
                                       class="p-2 rounded-lg text-ink/30 hover:text-leaf-600 hover:bg-leaf-600/5 transition-all duration-200 hover:scale-105" title="Edit kategori">
                                    ...
                                    <form action="{{ route('admin.kategori.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kategori ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 rounded-lg text-ink/30 hover:text-red-500 hover:bg-red-50 transition-all duration-200 hover:scale-105" title="Hapus kategori">
