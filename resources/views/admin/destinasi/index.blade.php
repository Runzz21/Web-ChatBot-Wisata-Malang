...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
    <div>
        ...
        <h1 class="text-2xl font-display font-semibold text-forest-800 mt-1.5 tracking-tight">Data Destinasi</h1>
        <p class="text-forest-700/40 mt-1">Kelola destinasi wisata alam</p>
    </div>
    <a href="{{ route('admin.destinasi.create') }}" class="btn-solid text-sm group">
...
<div class="card-glass p-6">
    <form action="{{ route('admin.destinasi.index') }}" method="GET" class="mb-6 flex flex-col sm:flex-row gap-3">
        ...
        <select name="kategori" class="select-custom max-w-[200px]">
        ...
        @if(request('search') || request('kategori'))
            <a href="{{ route('admin.destinasi.index') }}" class="btn-ghost text-sm">Reset</a>
        @endif
    </form>
            ...
            <table class="w-full text-sm text-left">
                <thead>
                    <tr class="border-b border-forest-600/10">
                        ...
                        <th class="py-3.5 px-4 font-semibold text-forest-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Nama</th>
                        <th class="py-3.5 px-4 font-semibold text-forest-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Kategori</th>
                        <th class="py-3.5 px-4 font-semibold text-forest-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Lokasi</th>
                        <th class="py-3.5 px-4 font-semibold text-forest-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Harga</th>
                        <th class="py-3.5 px-4 font-semibold text-forest-700 font-mono text-[0.6875rem] tracking-wider whitespace-nowrap">Status</th>
                ...
                <tbody>
                    @forelse($destinasi as $item)
                        <tr class="border-b border-forest-600/5 hover:bg-forest-600/[0.03] transition-colors duration-200">
                            ...
                            <td class="py-3.5 px-4 font-medium text-forest-700 whitespace-nowrap">{{ $item->nama }}</td>
                            <td class="py-3.5 px-4 whitespace-nowrap">
                                @if($item->kategori)
                                <span class="inline-flex items-center px-2.5 py-0.5 text-[0.6875rem] font-medium rounded-full" style="background: {{ $item->kategori->warna_badge ?? '#2D4A3E' }}14; color: {{ $item->kategori->warna_badge ?? '#2D4A3E' }};">
                                    {{ $item->kategori->nama_kategori }}
                                </span>
                                @else
                                <span class="text-forest-700/30">-</span>
                                @endif
                            </td>
                            <td class="py-3.5 px-4 text-forest-700/50 whitespace-nowrap">{{ $item->lokasi }}</td>
                            <td class="py-3.5 px-4 text-forest-700/50 font-mono text-[0.75rem] whitespace-nowrap">
                                @if($item->harga_tiket > 0)
                                    Rp{{ number_format($item->harga_tiket, 0, ',', '.') }}
                                @else
                                    <span class="text-forest-700/30">Gratis</span>
                                @endif
                            </td>
                            <td class="py-3.5 px-4 whitespace-nowrap">
                                @if($item->status_aktif)
                                    <span class="inline-flex items-center px-2.5 py-0.5 text-[0.6875rem] font-medium rounded-full" style="background: rgba(45, 74, 62, 0.08); color: #2D4A3E;">Aktif</span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 text-[0.6875rem] font-medium rounded-full" style="background: rgba(239, 68, 68, 0.08); color: #ef4444;">Nonaktif</span>
                                @endif
                            </td>
                            <td class="py-3.5 pl-4 whitespace-nowrap">
                                <div class="flex items-center gap-0.5">
                                    <a href="{{ route('admin.destinasi.edit', $item) }}"
                                       class="p-2 rounded-lg text-forest-700/30 hover:text-forest-600 hover:bg-forest-600/5 transition-all duration-200 hover:scale-105" title="Edit destinasi">
                                    ...
                                    <form action="{{ route('admin.destinasi.toggle-status', $item) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit"
                                                class="p-2 rounded-lg transition-all duration-200 hover:scale-105 {{ $item->status_aktif ? 'text-forest-700/30 hover:text-forest-600 hover:bg-forest-600/5' : 'text-forest-600 hover:bg-forest-600/5' }}"
                                                title="{{ $item->status_aktif ? 'Nonaktifkan' : 'Aktifkan' }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M8 11V7a4 4 0 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z"/></svg>
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.destinasi.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus destinasi ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 rounded-lg text-forest-700/30 hover:text-red-500 hover:bg-red-50 transition-all duration-200 hover:scale-105" title="Hapus destinasi">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-20 text-center text-forest-700/50">
                                ...
                                <p class="font-display font-semibold text-forest-600">Belum ada data destinasi</p>
                                <p class="text-sm text-forest-700/30 mt-1">Mulai dengan menambah destinasi baru</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
