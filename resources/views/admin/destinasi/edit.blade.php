...
<div class="mb-8 reveal">
    ...
    <h1 class="text-2xl font-display font-semibold text-forest-800 mt-1.5 tracking-tight">Edit Destinasi</h1>
    <p class="text-forest-700/50 mt-1">Perbarui data destinasi wisata alam</p>
</div>
    ...
    <form action="{{ route('admin.destinasi.update', $destinasi) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="md:col-span-2">
                ...
                @if($destinasi->foto_utama)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $destinasi->foto_utama) }}" alt="{{ $destinasi->nama }}"
                             class="w-48 h-32 object-cover rounded-xl" style="border: 1px solid rgba(45, 74, 62, 0.06);">
                    </div>
                @endif
                ...
                <p class="mt-1 text-xs text-forest-700/40">Kosongkan jika tidak ingin mengubah foto</p>
                @error('foto_utama')
                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
