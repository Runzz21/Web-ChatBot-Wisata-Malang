...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 reveal">
    <div>
        ...
        <h1 class="text-2xl font-display font-semibold text-ink mt-1.5 tracking-tight">Galeri — {{ $destinasi->nama }}</h1>
        <p class="text-ink/50 mt-1">Kelola foto destinasi wisata</p>
    </div>
...
<div class="card-glass p-6 mb-6 reveal" style="transition-delay: 0.1s">
    <h2 class="text-lg font-display font-semibold text-ink mb-4">Upload Foto Baru</h2>
        ...
        <div class="flex flex-col sm:flex-row gap-3 items-start">
            <div class="flex-1 w-full">
                <input type="file" name="foto[]" id="foto" multiple accept="image/*"
                       class="input-field @error('foto.*') border-red-400 @enderror p-2">
                @error('foto.*')
...
<div class="card-glass p-6 reveal" style="transition-delay: 0.2s">
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-display font-semibold text-ink">Daftar Foto</h2>
        @if($destinasi->galeri->count() > 0)
            <form action="{{ route('admin.galeri.urutan', $destinasi) }}" method="POST" id="formUrutan">
                ...
                <button type="submit" class="btn-primary text-sm" id="simpanUrutanBtn" style="display:none;">
                    Simpan Urutan
                </button>
            </form>
        @endif
    ...
    @if($destinasi->galeri->count() > 0)
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4" id="galeriGrid">
            @foreach($destinasi->galeri->sortBy('urutan') as $foto)
                <div class="relative group rounded-xl overflow-hidden" style="background: rgba(14, 29, 24, 0.03); border: 1px solid rgba(14, 29, 24, 0.06);" data-id="{{ $foto->id }}">
                    <img src="{{ asset('storage/' . $foto->url_foto) }}" alt="Foto {{ $destinasi->nama }}"
                         class="w-full h-40 object-cover">
                    <div class="absolute inset-0 bg-ink/0 group-hover:bg-ink/40 transition-all flex items-center justify-center gap-2">
                    ...
                    <div class="absolute top-2 left-2 font-mono text-[0.6875rem] px-2 py-1 rounded-lg backdrop-blur-sm"
                         style="background: rgba(14, 29, 24, 0.6); color: #FDFBF7;">
                        {{ $foto->urutan }}
                    </div>
                    <div class="absolute top-2 right-2 cursor-move rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity handle shadow-warm"
                        ...
                        <svg class="w-4 h-4 text-ink/50" fill="currentColor" viewBox="0 0 24 24"><path d="M8 6h2v2H8V6zm6 0h2v2h-2V6zM8 11h2v2H8v-2zm6 0h2v2h-2v-2zm-6 5h2v2H8v-2zm6 0h2v2h-2v-2z"/></svg>
                    </div>
                </div>
            @endforeach
        </div>
    @else
        <div class="text-center py-16 text-ink/50">
            <div class="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center" style="background: rgba(14, 29, 24, 0.05);">
                <svg class="w-8 h-8 text-ink/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            </div>
            <p class="font-display font-semibold text-leaf-700">Belum ada foto</p>
