    ...
    @if($rekomendasi && $rekomendasi->count())
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="card-hover group">
                ...
                <div class="p-4">
                    <h4 class="font-display font-semibold text-primary-800 mb-1 group-hover:text-accent transition-colors">{{ $item->nama }}</h4>
                    <div class="flex items-center font-mono text-[0.6875rem] text-[#5A6B55] tracking-wide mb-2">
                        <svg class="w-3.5 h-3.5 mr-1 text-[#5A6B55]/60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        {{ $item->lokasi }}
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="font-mono text-[0.6875rem] text-[#5A6B55] tracking-wide">{{ $item->formatted_jarak ?? ($item->jarak_km ? $item->jarak_km . ' km' : '-') }}</span>
                        <span class="font-semibold text-accent font-mono text-[0.75rem] tracking-wide">{{ $item->formatted_harga ?? 'Gratis' }}</span>
                    </div>
                    <a href="{{ route('destinasi.show', $item->nama) }}"
                       class="mt-3 w-full inline-flex items-center justify-center px-3 py-2 bg-primary-700 text-white text-xs font-semibold rounded-xl hover:bg-primary-800 transition-colors shadow-lereng-sm">
                        Lihat Detail
