        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16">
                ...
                <div class="lg:col-span-2 space-y-6 reveal-left">
                    <div>
                        <span class="text-xs font-medium tracking-widest uppercase text-forest-700/40">Kontak</span>
                        <h2 class="text-2xl sm:text-3xl font-display font-bold text-forest-800 mt-3 mb-2 tracking-tight">Informasi Kontak</h2>
                        <p class="text-forest-700/50">Jangan ragu untuk menghubungi kami melalui kontak di bawah ini</p>
                    </div>
                    ...
                    <div class="card-glass p-5 flex items-start space-x-4 group hover:bg-cream-50" style="transition-delay: {{ $loop->index * 0.1 }}s">
                        <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-105"
                            ...
                            <svg class="w-5 h-5 text-forest-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="{{ $item['icon'] }}"/></svg>
                        </div>
                        <div>
                            <h3 class="font-display font-semibold text-forest-700">{{ $item['title'] }}</h3>
                            @if(isset($item['link']) && $item['link'])
                            <a href="{{ Str::startsWith($item['content'], '+62') ? 'tel:' . str_replace([' ', '-'], '', $item['content']) : 'mailto:' . $item['content'] }}"
                               class="text-sm text-forest-600 hover:text-gold-600 transition-colors duration-300 mt-0.5 block">
                                {{ $item['content'] }}
                            </a>
                            @else
                            <p class="text-sm text-forest-700/60 mt-0.5">{{ $item['content'] }}</p>
                            @endif
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="lg:col-span-3 reveal-right">
                    <div class="card-glass p-8">
                        <h2 class="text-xl font-display font-bold text-forest-800 mb-2">Kirim Pesan</h2>
                        <p class="text-sm text-forest-700/50 mb-8">Isi form di bawah ini untuk mengirimkan pesan kepada kami</p>

                        <form>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                                <div>
                                    <label for="name" class="block text-sm font-medium text-forest-700 mb-2">Nama Lengkap</label>
                                    <input type="text" id="name" name="name"
                                           class="input-custom"
                                           placeholder="Masukkan nama lengkap">
                                </div>
                                <div>
                                    <label for="email" class="block text-sm font-medium text-forest-700 mb-2">Email</label>
                            ...
                            <div class="mb-5">
                                <label for="subject" class="block text-sm font-medium text-forest-700 mb-2">Subjek</label>
                                <input type="text" id="subject" name="subject"
                                       class="input-custom"
                                       placeholder="Masukkan subjek pesan">
                            </div>
                            <div class="mb-8">
                                <label for="message" class="block text-sm font-medium text-forest-700 mb-2">Pesan</label>
                                <textarea id="message" name="message" rows="5"
                                          class="input-custom resize-none"
                                          placeholder="Tulis pesan kamu di sini..."></textarea>
                            </div>
                            <button type="submit"
                                    class="btn-solid group">
