...
<head>
    ...
    <title>@yield('title', 'Dashboard') | Forest Admin</title>
...
<body class="bg-cream-50 text-forest-800 antialiased">
    <div class="min-h-screen flex">
        {{-- Sidebar Desktop --}}
        <aside class="w-64 bg-forest-800 text-cream-50/80 flex-shrink-0 hidden lg:flex lg:flex-col relative overflow-hidden">
            ...
            <div class="relative z-10 px-5 pt-5 pb-4 border-b border-cream-50/5">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 group">
                    ...
                    <div>
                        <p class="font-display font-semibold text-cream-50 text-base tracking-tight">Forest</p>
                        <p class="text-[0.6875rem] text-cream-50/40 font-mono tracking-widest uppercase">Admin Panel</p>
                    </div>
                </a>
            </div>
            ...
            <nav class="relative z-10 flex-1 px-3 py-5 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route(str_replace('*', 'index', $item['route'])) }}"
                   class="flex items-center px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-300 relative group {{ $isActive ? 'bg-cream-50/10 text-cream-50' : 'text-cream-50/45 hover:bg-cream-50/5 hover:text-cream-50/70' }}">
                    @if($isActive)
            ...
            <div class="relative z-10 p-4 border-t border-cream-50/5">
                <div class="flex items-center mb-3 group cursor-pointer">
                    <div class="w-8 h-8 rounded-lg bg-gold-500/15 flex items-center justify-center text-gold-500 font-bold text-sm font-mono transition-transform duration-300 group-hover:scale-105">
                        {{ substr(session('admin_username'), 0, 1) }}
                    </div>
                    <div class="ml-3 flex-1 min-w-0">
                        <p class="text-sm text-cream-50/60 font-medium truncate">{{ session('admin_username') }}</p>
                        <p class="text-[0.625rem] text-cream-50/30 font-mono tracking-wider">Administrator</p>
                    </div>
                </div>
                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="flex items-center w-full px-3 py-2 text-xs text-cream-50/30 hover:text-red-400 hover:bg-red-500/5 rounded-lg transition-all duration-300">
        ...
        <div class="flex-1 flex flex-col min-h-screen">
            ...
            <header class="lg:hidden glass-nav shadow-warm">
                <div class="flex items-center justify-between px-4 py-3">
                    <button x-data @click="$refs.mobileMenu.classList.toggle('hidden')" class="text-forest-600 p-2 hover:bg-forest-600/5 rounded-lg transition-colors">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="font-display font-semibold text-forest-700 tracking-tight">Forest</span>
                    </div>
                    <div class="w-6"></div>
                </div>
            </header>
            ...
            {{-- Mobile Menu --}}
            <div x-ref="mobileMenu" class="hidden lg:hidden bg-cream-50/95 backdrop-blur-lg border-b border-forest-600/10 shadow-warm">
                <nav class="px-2 py-3 space-y-1">
                    @foreach($navItems as $item)
                    @php $isActive = request()->routeIs($item['route']); @endphp
                    <a href="{{ route(str_replace('*', 'index', $item['route'])) }}"
                       class="flex items-center px-4 py-2.5 rounded-lg text-sm font-medium transition-colors {{ $isActive ? 'text-forest-600 bg-forest-600/5' : 'text-forest-700/60 hover:bg-forest-600/5' }}">
                    ...
                    @endforeach
                    <hr class="my-2 border-forest-600/10">
                    <form action="{{ route('admin.logout') }}" method="POST">
                        @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            @if(session('success'))
                Swal.fire({ icon: 'success', title: 'Berhasil!', text: '{{ session('success') }}', timer: 3000, showConfirmButton: false, background: '#F5F0E8', color: '#152A23', confirmButtonColor: '#2D4A3E' });
            @endif
            @if(session('error'))
                Swal.fire({ icon: 'error', title: 'Gagal!', text: '{{ session('error') }}', background: '#F5F0E8', color: '#152A23', confirmButtonColor: '#2D4A3E' });
            @endif
        });
    </script>
</body>
